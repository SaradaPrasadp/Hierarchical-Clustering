...
#we'll cluster as per the cusomer's loyalty

rect.hclust(fit, k = 4, border = "red")

customer_category <- as.matrix(groups)

final <- data.frame(customer_category, tc)

aggregate(tc, by = list(final$customer_category), FUN = mean)
